...
export const sendOTP = async (to, otp, type = "login") => {
  const isLogin = type === "login";

  await resend.emails.send({
    from: process.env.RESEND_FROM,
    to,
    subject: isLogin ? "Login OTP - We Talk" : "Password Reset OTP - We Talk",
    html: `
      <div style="font-family: sans-serif; max-width: 400px; margin: auto; padding: 2rem; background: #1e1b2e; border-radius: 12px; color: #eee;">
        ...
        <p style="color: #aaa; margin-bottom: 1.5rem;">
          ${isLogin ? "Your 2FA login OTP" : "Your password reset OTP"}
        </p>
